...
import SwiftUI
    /// Moved to `RailEntryBuilder`, so the Linux panel builds the rail from
    /// the same rules rather than from a copy of them. See that file.
    private var entries: [RailEntry] {
        RailEntryBuilder.entries(settings: settings, store: store, minute: minute)
    }
...
struct FloatingUsagePanelView: View {
    ...
    private var panelAlong: CGFloat {
        let panel = PanelLayout.size(for: placement.edge, notchSize: placement.notch?.size)
    ...
    private func isOverContent(_ point: CGPoint) -> Bool {
        ...
        let panel = PanelLayout.size(for: edge, notchSize: placement.notch?.size)
